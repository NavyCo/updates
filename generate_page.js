var fs, li, lists, page, pg;

fs = require('fs');

li = function(title, text, date) {
  var _o, timform;
  date = typeof date === 'object' ? date : new Date(date);
  timform = function(str) {
    return str.replace(/(\d{2}):(\d{2}):(\d{2}) \w+-\d+ \([\w\s]{1,}\)/, "$1:$2");
  };
  _o = [];
  _o.push("\t\t\t\t<li>");
  _o.push(["\t\t\t\t\t<time class=\"cbp_tmtime\" datetime=\"" + (date.toISOString()) + "\">", "<span>" + (date.toLocaleDateString()) + "</span> <span>" + (timform(date.toTimeString())) + "</span>", "</time>"].join(''));
